...
public class IronFurnacesJEIPlugin implements IModPlugin {
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        ...
        if (level != null) {
            Level burnLevel = level;
            Minecraft mc = Minecraft.getInstance();
            if (mc.hasSingleplayerServer()) {
                burnLevel = mc.getSingleplayerServer().overworld();
            }
            for (Item item : BuiltInRegistries.ITEM.stream().toList()) {
                ItemStack stack = new ItemStack(item);
                int burnTime = BlockIronFurnaceTileBase.getBurnTime(stack, RecipeType.SMELTING, burnLevel);
                if (burnTime > 0) {

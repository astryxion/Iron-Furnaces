...
public abstract class BlockIronFurnaceScreenBase<T extends BlockIronFurnaceContainerBase> extends AbstractContainerScreen<T> {
    ...
    public ResourceLocation GUI = ResourceLocation.fromNamespaceAndPath(IronFurnaces.MOD_ID, "textures/gui/furnace.png");
    ...
    public void setShowConfig(int value)
    {
        ...
        ClientPlayNetworking.send(new PacketShowConfig(value));
    }
    ...
    protected void renderLabels(GuiGraphics matrix, int mouseX, int mouseY) {
        ...
        if (this.getMenu().getIsFactory())
            matrix.drawString(font, name, this.imageWidth / 2 - this.minecraft.font.width(name.getString()) / 2, -10, 16777215, false);
        else
            matrix.drawString(font, name, this.getMenu().getIsFurnace() ? 7 + this.imageWidth / 2 - this.minecraft.font.width(name.getString()) / 2 : this.imageWidth / 2 - this.minecraft.font.width(name.getString()) / 2, 6, 4210752, false);

        matrix.drawString(font, this.playerInv.getDisplayName(), 7, this.imageHeight - 93, 4210752, false);
    ...
    private void bg(GuiGraphics matrix, int relX, int relY)
    {
        if (!this.getMenu().getAugmentGUI())
        {
            if (this.getMenu().getIsFactory())
            {
                matrix.blit(GUI_FACTORY, relX, relY, 0, 0, this.imageWidth, this.imageHeight);

            }
            if (this.getMenu().getIsGenerator())
            {
                matrix.blit(GUI_GENERATOR, relX, relY, 0, 0, this.imageWidth, this.imageHeight);
        ...
        {
            matrix.blit(GUI_AUGMENTS, relX, relY, 0, 0, this.imageWidth, this.imageHeight);
    ...
    protected void renderFurnaceBg(GuiGraphics matrix)
    {
        if (getMenu().getIsFurnace() && !getMenu().getAugmentGUI())
        {
            ...
            if (this.getMenu().isBurning()) {
                ...
                matrix.blit(GUI, leftPos + 56, topPos + 36 + 12 - i, 176, 12 - i, 14, i + 1);
            }

            i = this.getMenu().getCookScaled(24);
            matrix.blit(GUI, leftPos + 79, topPos + 34, 176, 14, i + 1, 16);
        }
    }
    ...
    protected void renderGeneratorBg(GuiGraphics matrix)
    {
        if (getMenu().getIsGenerator() && !getMenu().getAugmentGUI())
        {
            ...
            if (this.getMenu().isGeneratorBurning()) {
                ...
                matrix.blit(GUI_GENERATOR, leftPos + 56, topPos + 23 + 12 - i, 176, 12 - i, 14, i + 1);
            }
    ...
    protected void renderFactoryBg(GuiGraphics matrix)
    {
        if (getMenu().getIsFactory() && !getMenu().getAugmentGUI())
        {
            ...
            for (int j = 0; j < getMenu().getFactoryCooktimeSize(); j++)
            {
                ...
                matrix.blit(GUI_FACTORY, leftPos + 29 + (21 * j), topPos + 27, 176, 0, 15, i + 1);
            }
        }
    }
    ...
    protected void addSlots(GuiGraphics matrix, int amount)
    {
        if (this.getMenu().getIsFactory())
        {
            if (amount > 0)
            {
                matrix.blit(GUI_FACTORY, leftPos + 48, topPos + 5, 176, 64, 18, 67);
                matrix.blit(GUI_FACTORY, leftPos + 111, topPos + 5, 176, 64, 18, 67);
                if (amount == 2)
                {
                    matrix.blit(GUI_FACTORY, leftPos + 27, topPos + 5, 176, 64, 18, 67);
                    matrix.blit(GUI_FACTORY, leftPos + 132, topPos + 5, 176, 64, 18, 67);
                }
            }
    ...
    private void addInventoryButtons(GuiGraphics matrix, int mouseX, int mouseY) {
        if (!showInventoryButtons()) {
            matrix.blit(WIDGETS, leftPos - 20, topPos + 4, 0, 28, 23, 26);
        } else if (showInventoryButtons()) {
            matrix.blit(WIDGETS, leftPos - 56, topPos + 4, 0, 54, 59, 107);
    ...
    private void blitIO(GuiGraphics matrix, int mouseX, int mouseY) {
        ...
        if (input || both) {
            ...
            {
                matrix.blit(WIDGETS, leftPos + 55, topPos + 16, 0, 171, 18, 18);
            }
            if (getMenu().getIsFactory())
            {
                matrix.blit(WIDGETS, leftPos + 69, topPos + 5, 0, 171, 18, 18);
                matrix.blit(WIDGETS, leftPos + 90, topPos + 5, 0, 171, 18, 18);
                if (getMenu().getTier() > 0)
                {
                    matrix.blit(WIDGETS, leftPos + 48, topPos + 5, 0, 171, 18, 18);
                    matrix.blit(WIDGETS, leftPos + 111, topPos + 5, 0, 171, 18, 18);
                    if (getMenu().getTier() > 1)
                    {
                        matrix.blit(WIDGETS, leftPos + 27, topPos + 5, 0, 171, 18, 18);
                        matrix.blit(WIDGETS, leftPos + 132, topPos + 5, 0, 171, 18, 18);
                    }
                }
            }
        ...
        if (output || both) {
            if (getMenu().getIsFurnace())
            {
                matrix.blit(WIDGETS, leftPos + 111, topPos + 30, 0, 203, 26, 26);
            }
            if (getMenu().getIsFactory())
            {
                matrix.blit(WIDGETS, leftPos + 69, topPos + 54, 36, 171, 18, 18);
                matrix.blit(WIDGETS, leftPos + 90, topPos + 54, 36, 171, 18, 18);
                if (getMenu().getTier() > 0)
                {
                    matrix.blit(WIDGETS, leftPos + 48, topPos + 54, 36, 171, 18, 18);
                    matrix.blit(WIDGETS, leftPos + 111, topPos + 54, 36, 171, 18, 18);
                    if (getMenu().getTier() > 1)
                    {
                        matrix.blit(WIDGETS, leftPos + 27, topPos + 54, 36, 171, 18, 18);
                        matrix.blit(WIDGETS, leftPos + 132, topPos + 54, 36, 171, 18, 18);
                    }
                }
            }
        }
        if (fuel) {
            if (getMenu().getIsFurnace())
            {
                matrix.blit(WIDGETS, leftPos + 55, topPos + 52, 18, 171, 18, 18);
            }
            if (getMenu().getIsGenerator())
            {
                matrix.blit(WIDGETS, leftPos + 55, topPos + 39, 18, 171, 18, 18);
            }
        }
    }
    ...
    protected void clickInvButton(double mouseX, double mouseY, FurnaceGuiButton button, int buttonid, int setting, int index, boolean shift)
    {
        ...
        if (shift && frontButton.hovering(mouseX, mouseY))
        {
            ...
            {
                ClientPlayNetworking.send(new PacketFurnaceSettings(getMenu().getPos().getX(), getMenu().getPos().getY(), getMenu().getPos().getZ(), i, 0));
            }
        }
